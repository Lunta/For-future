...
bool CMainScene::ProcessInput()
{
	...
	if (!bProcessedByScene)
	{
		...
		if (GetKeyboardState(pKeyBuffer))
		{
			...
			if (pKeyBuffer['3'] & 0xF0) { bControll = true; m_bImmune = true;}
			if (pKeyBuffer['4'] & 0xF0) { bControll = true; m_bImmune = false; }
			if (pKeyBuffer['5'] & 0xF0) { bControll = true; m_bButton5 = true; }
			if (pKeyBuffer['6'] & 0xF0) { bControll = true; m_bButton6 = true; }
		...
		if (!bControll)
		{
			...
			{
				m_Embient += 0.1f;
				if (m_Embient > 1.0f) m_Embient = 1.0f;
				m_pLight->SetLightAmbient(CLight::LightTag::Light0, m_Embient, m_Embient, m_Embient);
				m_bButton5 = false;
			}
			if (m_bButton6)
			{
				m_Embient -= 0.1f;
				if (m_Embient < 0.0f) m_Embient = 0.0f;
				m_pLight->SetLightAmbient(CLight::LightTag::Light0, m_Embient, m_Embient, m_Embient);
				m_bButton6 = false;
			}
		}
