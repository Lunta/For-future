...
bool CPractice24::OnProcessingKeyboardMessage(HWND hWnd, UINT nMessageID, WPARAM wParam, LPARAM lParam)
{
	switch (nMessageID)
	{
	case WM_KEYDOWN:
...
bool CPractice24::OnProcessingMouseMessage(HWND hWnd, UINT nMessageID, WPARAM wParam, LPARAM lParam)
{
	switch (nMessageID)
	{
	case WM_LBUTTONDOWN:
...
bool CPractice24::OnProcessingWindowMessage(HWND hWnd, UINT nMessageID, WPARAM wParam, LPARAM lParam)
{
	switch (nMessageID)
	{
	case WM_SIZE:
...
void CPractice24::BuildPractice(CMainScene * CurrentScene, HWND hWnd, CurrentPractice tag)
{
	...
	m_fallCube[0].Scale(30.0f, 30.0f, 30.0f);
	m_fallCube[0].Translate(0.0f, 0.0f, 40.0f);
	m_fallCube[0].SetColor(200, 0, 50);
	m_fallCube[1].Scale(20.0f, 20.0f, 20.0f);
	m_fallCube[1].SetColor(50, 200, 0);
	m_fallCube[2].Scale(10.0f, 10.0f, 10.0f);
	m_fallCube[2].Translate(0.0f, 0.0f, -20.0f);
	m_fallCube[2].SetColor(0, 50, 200);
	
}

void CPractice24::Rendering()
{
...
void CPractice24::Update(float fTimeElapsed)
{
	ProcessInput();
	...
	for (int i = 0; i < 3; i++)
	{
		m_fallCube[i].Translate(
			70.0f*cosf((3.0f / 2.0f) * PI - ((m_angle_z * PI) / 180.0f)) * fTimeElapsed,
			70.0f*sinf((3.0f / 2.0f) * PI - ((m_angle_z * PI) / 180.0f)) * fTimeElapsed,
			0.0f);
	...
	for (int i = 0; i < 6; i++)
	{
		if (m_Surface[i].isCollide(m_fallCube[0].GetCenter() - 15.0f * m_Surface[i].Normal))
			m_fallCube[0].Translate(70.0f*m_Surface[i].Normal.x * fTimeElapsed, 70.0f*m_Surface[i].Normal.y * fTimeElapsed, 70.0f*m_Surface[i].Normal.z * fTimeElapsed);
		if (m_Surface[i].isCollide(m_fallCube[1].GetCenter() - 10.0f * m_Surface[i].Normal))
			m_fallCube[1].Translate(70.0f*m_Surface[i].Normal.x * fTimeElapsed, 70.0f*m_Surface[i].Normal.y * fTimeElapsed, 70.0f*m_Surface[i].Normal.z * fTimeElapsed);
		if (m_Surface[i].isCollide(m_fallCube[2].GetCenter() - 5.0f * m_Surface[i].Normal))
			m_fallCube[2].Translate(70.0f*m_Surface[i].Normal.x * fTimeElapsed, 70.0f*m_Surface[i].Normal.y * fTimeElapsed, 70.0f*m_Surface[i].Normal.z * fTimeElapsed);
	}
}

bool CPractice24::ProcessInput()
{
	...
	if (!bProcessedByScene)
	{
		...
		if (GetKeyboardState(pKeyBuffer))
		{
			...
			else if (pKeyBuffer['N'] & 0xF0) { bControll = true; m_pScene->ChangePractice(CPractice::CurrentPractice::Practice_25); }
			else if (pKeyBuffer['B'] & 0xF0) { bControll = true; m_pScene->ChangePractice(CPractice::CurrentPractice::Practice_23); }
