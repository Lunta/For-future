...
class SpriteAnimation:
    ...
    def set_state_delay(self, state_index, execute_delay, action_delay, exit_delay):
        self._m_StateDelay[self._m_SpriteState_List.index(state_index)][self._m_StateState.execute] = execute_delay
        self._m_StateDelay[self._m_SpriteState_List.index(state_index)][self._m_StateState.action] = action_delay
        self._m_StateDelay[self._m_SpriteState_List.index(state_index)][self._m_StateState.exit] = exit_delay
    ...
    def update_state(self, state_name):
        ...
        # 퇴장 상태로 변경하지 않고 갱신
        if self.get_current_state_state() is self._m_StateState.execute:
            ...
            # 이미 퇴장중이면 취소
            if self.get_current_state_state() is self._m_StateState.exit:
                return None
    ...
    def get_current_state_state(self):
        ...
        # 진입 상태로 초기화
        state_state = self._m_StateState.execute
        ...
        if self._m_CurrentState.get_currentframe_x() >= \
            ...
            state_state = self._m_StateState.exit
        # 진행상태일 경우
