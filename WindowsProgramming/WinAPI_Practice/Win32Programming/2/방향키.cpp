...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMessage, WPARAM wParam, LPARAM lParam)
{
	HDC hdc; //디바이스컨텍스트 핸들
	PAINTSTRUCT ps;
	static HBRUSH hRBrush, hGBrush, oldBrush;
	static int width, height, wp, color, check ;
	int i, j;
	...
	case WM_KEYDOWN:
		...
		else if ((wParam == 'R' || wParam == 'r') && check == 0)
		{
			if (wp == 'r') wp = 0;
			else wp = 'r';
		}
	...
	case WM_PAINT:
		...
		if (wp == 'r') {
			for (i = 0; i < 4; i++)
				for (j = 0; j < 4; j++) {
					Ellipse(hdc, Width*i, Height*j, Width*(i + 1), Height*(j + 1));
				}
			if(color == 0) oldBrush = (HBRUSH)SelectObject(hdc, hRBrush);
			else if (color == 1) oldBrush = (HBRUSH)SelectObject(hdc, hGBrush);
			Ellipse(hdc, Width*width, Height*height, Width*(width + 1), Height*(height + 1));
		...
		} else 
		{
			...
			Rectangle(hdc, Width*width, Height*height, Width*(width + 1), Height*(height + 1));
