...
void DrawTextRect(HDC hdc, RECT rect, int xcount, int ycount);
...
{ 
	...
	case WM_PAINT:
		...
		//TextOut(hdc, x, y, _T(str), lstrlen(_T(str) )  );
		//DrawText(hdc, _T(str), -1, &rect, DT_Flag);
		
		for (ycount = 0; ycount < 10; ycount++)
			for (xcount = ycount; xcount < 19-ycount; xcount++)
				DrawTextRect(hdc, rect, xcount, ycount);
...
void DrawTextRect(HDC hdc, RECT rect, int xcount, int ycount)
{
	...
	DrawText(hdc, str , 7, &rect, DT_CENTER | DT_VCENTER );
}