...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMessage, WPARAM wParam, LPARAM lParam)
{
	...
	static TCHAR temp[10][100] = { 0 };
	static SIZE size;
	static int count, linecount, wp, tempn;
	TCHAR stemp[100] = { 0 };
	int i, j;
	...
	case WM_CHAR:
		...
		else if (wParam == '\\') //home
		{
			...
			linecount = rand() % 10;
			for (j = 0; j < count; j++)
				if(str[linecount][j] == '\0' || str[linecount][j] == ' ')
					str[linecount][j] = ' ';
			str[linecount][count] = '\0';
		}
	...
	case WM_KEYDOWN:
		...
		else if (wParam == VK_DELETE)
		{
			...
			{
				for (j = 0; j < count; j++)
					str[linecount][j] = str[linecount][j + 1];
				str[linecount][--count] = '\0';
			}
		}
		else if (wParam == VK_CONTROL) //islower : 소문자판별 isupper : 대문자 true !=0 (TCHAR)toupper/(TCHAR)tolower : 대문자/소문자로 변환
		{
			for (i = 0; i < 10; i++)
				for (j = 0; j < 100; j++)
				{
					if (islower(str[i][j]) != 0) str[i][j] = (TCHAR)toupper(str[i][j]);
					else if (isupper(str[i][j]) != 0) str[i][j] = (TCHAR)tolower(str[i][j]);
				}
		}
		else if (wParam == VK_DOWN )
		{
			tempn = 0;
			count = 0;
			for (i = 0; i < 8; i++) _tcscpy(temp[i], str[i]);
			for (i = 0; i < 10; i++)
			{
				for (j = 0; j < (lstrlen(str[i]) / 10) + 1; j++)
				{
					if (lstrlen(str[linecount]) != 0 && linecount < 9) linecount++;
					if (lstrlen(str[tempn]) > 10) {
						_tcscpy(str[i + j], temp[tempn] + 10 * j);
						count++;
					}
				}
				...
				for (j = 0; j < 100; j++)
				{
					if(str[i][j] != '\0')
						if(str[i][j] != ' ')
							...
							temp[i][j] = ' ';
					temp[i][_tcslen(temp[i])] = '\0';
					TextOut(hdc, 0, i * 20, temp[i], lstrlen(temp[i]));
				}
			}
		else 
		{
