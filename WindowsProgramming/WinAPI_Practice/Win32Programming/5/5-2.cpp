...
LRESULT CALLBACK WndProc(HWND hWnd, UINT iMessage, WPARAM wParam, LPARAM lParam)
{
	HDC hdc, memdc, bmpdc, copydc, olddc;
	PAINTSTRUCT ps;
	BITMAP bmp;
	HBITMAP blank;
	static HBITMAP hBitmap;
	static RECT rc, rect;
	static int flag, copy;
	static BOOL drag;
	static bool f;
	...
	case WM_PAINT:
		...
		if (flag == 0)
		{
			StretchBlt(hdc, rect.left, rect.top, rect.right - rect.left, rect.bottom - rect.top,
				bmpdc, rect.left, rect.top, (rect.right - rect.left), (rect.bottom - rect.top), SRCCOPY);
		}
		else if (flag == 1)
		{
			StretchBlt(hdc, rect.left, rect.top, rect.right - rect.left, rect.bottom - rect.top,
				bmpdc, rect.left, rect.top, (rect.right - rect.left)*0.8, (rect.bottom - rect.top)*0.8, SRCCOPY);
		}
		else if (flag == 2)
		{
			StretchBlt(hdc, rect.left, rect.top, rect.right - rect.left, rect.bottom - rect.top,
				bmpdc, rect.left, rect.top, (rect.right - rect.left)*0.6, (rect.bottom - rect.top)*0.6, SRCCOPY);
		}
		if (f == true)
		{
			if (copy == 0)
		...
		break;
	case WM_KEYDOWN:
		if (wParam == 'C') copy = flag; 
		else if (wParam == 'P')
		{
			if (f == false)f = true;
			else f = false;
		}
		else if (wParam == '1') flag = 1;
		else if (wParam == '2') flag = 2;
		else if (wParam == '0') flag = 0;
		else if (wParam == VK_LEFT) OffsetRect(&rect, -5,0);
		else if (wParam == VK_UP) OffsetRect(&rect, 0, -5);
		else if (wParam == VK_RIGHT) OffsetRect(&rect, 5, 0);
		else if (wParam == VK_DOWN) OffsetRect(&rect, 0, 5);
		InvalidateRect(hWnd, NULL, FALSE);
		break;
	case WM_CHAR:
		break;
