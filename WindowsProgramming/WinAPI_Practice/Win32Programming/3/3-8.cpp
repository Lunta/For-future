...
LRESULT CALLBACK WndProc(HWND hWnd, UINT iMessage, WPARAM wParam, LPARAM lParam)
{
	...
	static POINT mpoint, startp, line[FigureNum - 1];
	...
	case WM_TIMER:
		...
		case 1:
			...
			for (i = 1; i < FigureNum; i++)
			{
				line[i - 1].x = (int)(((e[i - 1].right - e[i - 1].left) / 2) * cos(radian));
				line[i - 1].y = (int)(((e[i - 1].right - e[i - 1].left) / 2) * sin(radian));
				radian += 0.1;
			}
			InvalidateRect(hWnd, NULL, TRUE);
			break;
	...
	case WM_PAINT:
		...
		for (i = 1; i < FigureNum; i++)
		{
			...
			MoveToEx(hdc, ((ellipse[i].left + ellipse[i].right) / 2) - ((ellipse[i].right - ellipse[i].left ) / 2), ((ellipse[i].top + ellipse[i].bottom) / 2) , NULL);
			LineTo(hdc, ((ellipse[i].left + ellipse[i].right) / 2) + ((ellipse[i].right - ellipse[i].left) / 2), ((ellipse[i].top + ellipse[i].bottom) / 2));
			MoveToEx(hdc, ((ellipse[i].left + ellipse[i].right) / 2), ((ellipse[i].top + ellipse[i].bottom) / 2) - ((ellipse[i].bottom - ellipse[i].top) / 2), NULL);
			LineTo(hdc, ((ellipse[i].left + ellipse[i].right) / 2), ((ellipse[i].top + ellipse[i].bottom) / 2) + ((ellipse[i].bottom - ellipse[i].top) / 2));
			oldBrush = (HBRUSH)SelectObject(hdc, GetStockObject(GRAY_BRUSH));
			Ellipse(hdc, e[i - 1].left, e[i - 1].top, e[i - 1].right, e[i - 1].bottom);
			MoveToEx(hdc, ((e[i - 1].left + e[i - 1].right) / 2 ) +line[i - 1].x, ((e[i - 1].top + e[i - 1].bottom) / 2)+line[i - 1].y, NULL);
			LineTo(hdc, ((e[i - 1].left + e[i - 1].right) / 2) -line[i - 1].x, ((e[i - 1].top + e[i - 1].bottom) / 2) -line[i - 1].y);
		}
...
/*실습 3-6에서 모든 원들은 원 내부에 바퀴틀을 가진다
주인공 원이 부딪치는 장애물 워은 시계방향 또는 반 시계방향으로 회전한다.*/