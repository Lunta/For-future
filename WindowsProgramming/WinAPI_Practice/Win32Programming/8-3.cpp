...
enum KindOfTile {
	// 1~6번 타일
	tnone = 0, tile1, tile2, tile3, tile4, tile5, tile6
};
enum KindOfObject {
	onone = 0, obj1, obj2 = 2
};
enum KindOfLoad {
	ltile = 0, lobject, lBK_AND_SIZE
};
typedef struct TILE
{
	RECT rc;
...
TILE * tNew();
TILE * tDelete(TILE * head);

OBJECT * oNew();
OBJECT * oDelete(OBJECT * head);
		...
		case ID_SAVE:
			...
			if (GetSaveFileName(&OFN) != 0) {
				...
				while (tmp != nullptr) {
					...
					wsprintf(wbuff, L"%4d %4d %4d %4d %4d \r\n",
						tmp->rc.left, tmp->rc.top, tmp->rc.right, tmp->rc.bottom, tmp->tilekind);
				...
				while (otmp != nullptr) {
					...
					memset(wbuff, 0, (STRMAX - 1) * sizeof(TCHAR));
					wsprintf(wbuff, L"%4d %4d %4d %4d %4d \r\n",
						otmp->rc.left, otmp->rc.top, otmp->rc.right, otmp->rc.bottom, otmp->objectkind);
					WideCharToMultiByte(CP_ACP, 0, wbuff, -1, buff, STRMAX - 1, NULL, NULL);
					WriteFile(hFile, buff, strlen(buff), &size, NULL);
					otmp = otmp->next;
				}
				...
				WideCharToMultiByte(CP_ACP, 0, wbuff, -1, buff, STRMAX - 1, NULL, NULL);
				WriteFile(hFile, buff, strlen(buff), &size, NULL);
				wsprintf(wbuff, L"%4d %4d %4d %4d %4d \r\n", mapWidth, mapHeight, tileWidth, tileHeight, cselection);
				WideCharToMultiByte(CP_ACP, 0, wbuff, -1, buff, STRMAX - 1, NULL, NULL);
				WriteFile(hFile, buff, strlen(buff), &size, NULL);
				CloseHandle(hFile);
			}
			...
			if (GetOpenFileName(&OFN) != 0) {
				...
				while (1) {
					...
					if (load == ltile) {
						...
						if (_wtoi(wbuff) == 0 && _wtoi(wbuff + 5) == 0 && _wtoi(wbuff + 10) == 0 && _wtoi(wbuff + 15) == 0 && _wtoi(wbuff + 20) == 0)
							load = lobject;
						else {
							for (int i = 0; i < 5; i++)
							{
								switch (i)
								{
								case 0: ttail->rc.left = _wtoi(wbuff + 5 * i); break;
								case 1: ttail->rc.top = _wtoi(wbuff + 5 * i); break;
								case 2: ttail->rc.right = _wtoi(wbuff + 5 * i); break;
								case 3: ttail->rc.bottom = _wtoi(wbuff + 5 * i); break;
								case 4: ttail->tilekind = _wtoi(wbuff + 5 * i); break;
								}
							}
						}
					}
					else if(load == lobject) {
						if (ohead == nullptr)
						{
							ohead = oNew();
							otail = ohead;
						...
						if (_wtoi(wbuff) == 0 && _wtoi(wbuff + 5) == 0 && _wtoi(wbuff + 10) == 0 && _wtoi(wbuff + 15) == 0 && _wtoi(wbuff + 20) == 0)
							...
							for (int i = 0; i < 5; i++)
							{
								switch (i)
								{
								case 0: otail->rc.left = _wtoi(wbuff + 5 * i); break;
								case 1: otail->rc.top = _wtoi(wbuff + 5 * i); break;
								case 2: otail->rc.right = _wtoi(wbuff + 5 * i); break;
								case 3: otail->rc.bottom = _wtoi(wbuff + 5 * i); break;
								case 4: otail->objectkind = _wtoi(wbuff + 5 * i); break;
								}
							}
						}
					}
					else if (load == lBK_AND_SIZE) {
						MultiByteToWideChar(CP_ACP, 0, buff, -1, wbuff, STRMAX - 1);
						for (int i = 0; i < 5; i++)
						{
							switch (i)
							{
							case 0: mapWidth = _wtoi(wbuff + 5 * i); break;
							case 1: mapHeight = _wtoi(wbuff + 5 * i); break;
							case 2: tileWidth = _wtoi(wbuff + 5 * i); break;
							case 3: tileHeight = _wtoi(wbuff + 5 * i); break;
							case 4: cselection = _wtoi(wbuff + 5 * i); break;
							}
						}
						SetDlgItemInt(hWnd, IDC_MAPWIDTH_EDIT, mapWidth, true);
						SetDlgItemInt(hWnd, IDC_MAPHEIGHT_EDIT, mapHeight, true);
